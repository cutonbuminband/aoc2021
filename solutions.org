...
#+begin_src jupyter-python
  values, counts = np.unique(np.loadtxt('data/6.txt', delimiter=',', dtype=int), return_counts=True)
  population = np.zeros(9)
  population[values] = counts
  def update(array):
      return np.roll(array, -1) + np.array([0, 0, 0, 0, 0, 0, array[0], 0, 0])
